...
#'
#' @description visualize some value on an ST grid using voronoi tesselation
#' @param object object of class spaceST
#' @param select.columns select columns of target matrix [default = NULL]
...
tessViz <- function(
  ...
  for (n in unique(all.coords$replicate)) {
    ...
    # Plot each column in Grayscale if select columns is notspecified
